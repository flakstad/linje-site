// Minimal Cloudflare Worker proxy for linje-site -> Linje /admin/signups.
//
...
export default {
  async fetch(request, env) {
    ...
    const upstream = await fetch(`${baseUrl}/admin/signups`, {
      method: "POST",
      headers: {
    ...
    if (!upstream.ok) {
      ...
      return json(
        upstream.status,
        { ok: false, error: "upstream error", upstream_status: upstream.status, body: text.slice(0, 500) },
        corsOrigin
      );
    }
